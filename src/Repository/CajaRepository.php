...
class CajaRepository extends ServiceEntityRepository
{
    ...
    public function getIngresos($desc = '', $desde = '', $hasta = '', $conTicket = true)
    {
        $query = $this->createQueryBuilder('c');
        if ($conTicket) {
            $query = $query->andWhere('c.llevaTicket = :ticket')->setParameter('ticket', '1');
        } else {
            $query = $query->andWhere('c.llevaTicket = :ticket')->setParameter('ticket', '0');
        }
        if ($desc != '') {
